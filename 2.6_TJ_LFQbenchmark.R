...
# Additional files might be moved to subfolders 
...
# mixed together in different percentages. A common example below.
...
# measured log2 fold-changes deviate from expectations,
# including e.g. global over or underestimation and errors.
...
# of multiple datasets in comparison for e.g. optimization studies.
...
# (fold-change underestimation, somewhat common in noisy mass spec data).
...
# matching a different species. (Potential misidentifications, inflated 
# precursor or protein group FDR)
# Also check for proper normalization and over- or underestimation of
# fold-changes on density plots.
...
# delete folders to allow fresh re-install. 
...
folder_input <- "//fileserver/mslab/Tobi/Template_R_Scripts/LFQbench_DE/000"


# Naming the 2 different benchmark sample mixtures as if they are conditions.
cond_ctr <- "LFQ_B"
cond_exp <- "LFQ_A"
...
# directly need to prevent impactful value clipping.
FC_max <- +3.2
FC_min <- -3.2
...
# the respective export functions.
...
# and retain only single-species entries.
# e.g. Prot1 <- f.LFQbenchmark.basics(Prot0, cond_ctr, cond_exp, col_ctr_Prot, col_exp_Prot)
