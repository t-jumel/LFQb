...
# 2) Select a folder location (variable "folder_input").
...
# Often measured with 3 technical replicates.

# Note: This script works directly on 2-species benchmarks,
# assuming Human and Yeast as species with the same orientation as the example.
# (Yeast up-regulated from Sample B to Sample A, Human stable;
# total Yeast concentrations lower than in the example are recommended)
...
# 4x down-regulated (non-log scale). The benchmarks reveal to what extent 
...
folder_input <- "//fileserver/project_jumel/All_LFQ_benchmarks/Reanalysis_external/Reanalysis_LFQbench_PXD028735/diaPASEF/diaPASEF03"
...
asymmetry <- data.frame(matrix(ncol = 0, nrow = 6))
asymmetry[, "Group"] <- c(rep("precursor", 3), 
                          rep("protein group", 3))
asymmetry[, "Species"] <- c("E.coli", "Human", "Yeast")
